...
const MarkdownRenderer: React.FC<Props> = ({ tt }) => {
  return (
    <div className="w-full prose-lg text-sm  h-full mx-auto overflow-hidden p-4">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        rehypePlugins={[rehypeRaw]}
        components={{
          ...
          ul: ({ node, ...props }) => (
            <ul className="list-disc text-lg list-inside ml-6" {...props} />
          ),
          ol: ({ node, ...props }) => (
            <ol className="list-decimal text-lg  ml-6" {...props} />
          ),
          li: ({ node, ...props }) => (
