  ...
  return (
    <div className={`h-screen shadow-md flex flex-col font-sans text-base transition-all duration-300 ${isOpen ? 'w-64' : ''}`}>
      ...
      <nav className={`flex-1 px-4 py-8 space-y-4 ${isOpen ? 'block' : 'hidden'}`}>
        <a href="#chats" className="flex items-center cursor-not-allowed text-zinc-900">
        ...
        <a href="#reports" className="flex items-center cursor-not-allowed text-zinc-900">
          <svg className="w-6 h-6 mr-3" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col">
          <button onClick={toggleAgents} disabled className="flex items-center cursor-not-allowed justify-between text-zinc-900 focus:outline-none">
